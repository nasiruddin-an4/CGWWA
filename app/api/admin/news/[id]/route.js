...
export async function PUT(request, { params }) {
  try {
    ...
    if (imageFile && imageFile.size > 0) {
      ...
      const existingDoc = await db.collection('news').findOne({ _id: id });
      if (existingDoc && existingDoc.featuredImage) {
        await deleteFromCloudinary(existingDoc.featuredImage);
      }
    }
    
    // Remove _id from data to avoid updating the immutable field
    const { _id, ...updateData } = data;
    
    await db.collection('news').updateOne({ _id: id }, { $set: updateData });

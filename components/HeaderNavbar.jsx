...
import {
  Search, Globe, Menu, X, ChevronDown, ExternalLink
} from 'lucide-react';
    ...
    <header 
      className={`w-full fixed top-0 z-50 transition-all duration-300 ${
        isTransparent 
          ...
          : 'bg-brandBlue border-b border-white/10 shadow-lg backdrop-blur-md'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
          ...
          <nav className="hidden lg:flex items-center space-x-2 font-medium text-sm tracking-wide">
            {navigationMenu
              .filter((item) => ['about', 'leadership', 'activities', 'news', 'gallery'].includes(item.id))
              .map((item) => {
